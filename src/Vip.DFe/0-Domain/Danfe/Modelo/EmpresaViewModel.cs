...
namespace Vip.DFe.Danfe.Modelo
{
    public class EmpresaViewModel
    {
        ...
        ///     Linha 1 do Endereço
        /// </summary>
        public string EnderecoLinha2 => $"{EnderecoBairro} - CEP: {DanfeHelper.FormatarCEP(EnderecoCep)}";
        ...
        public string EnderecoLinha3
        {
            get
            {
                var sb = new StringBuilder().Append(Municipio).Append(" - ").Append(EnderecoUf);

                if (!string.IsNullOrWhiteSpace(Telefone))
                    sb.Append(" Fone: ").Append(DanfeHelper.FormatarTelefone(Telefone));
